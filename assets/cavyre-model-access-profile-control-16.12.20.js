...
var temp=root.querySelector('[data-temp]');if(temp)temp.onclick=async function(){this.disabled=true;try{var out=await post('/api/agent/model-access',{model_id:id,action:'generate_temporary_password'});await openAccess(out.temporary_password||'',!!out.credential_verified)}catch(e){err.textContent=e.message||e;this.disabled=false}};
 ...
 var cp=root.querySelector('[data-copy]');if(cp)cp.onclick=async function(){var code=root.querySelector('code')?.textContent||'',email=a.email||a.suggested_email||'',text='Password Setup: https://www.maisondeveux.com/portal/password?mode=temp\nEmail: '+email+'\nTemporary Password: '+code+'\nOpen Password Setup, enter this temporary password, then choose a permanent password.';try{await navigator.clipboard.writeText(text);cp.textContent='Copied ✓'}catch(e){prompt('Copy login details:',text)}};
}
function inject(){
