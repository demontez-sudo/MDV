...
export async function requirePartnerPortal({user,organizationSlug='maison-de-veux'}){
  const admin=adminClient();
  const {data:organization,error:orgError}=await admin.from('organizations')
    .select('*').eq('slug',organizationSlug).maybeSingle();
  if(orgError)throw orgError;
  ...
  // Partner linkage is the canonical authorization bridge. organization_members is optional
  // because historical Mother Agency accounts were not consistently duplicated there.
  const {data:link,error:linkError}=await admin.from('partner_user_links')
    .select('id,partner_agency_id').eq('organization_id',organization.id).eq('user_id',user.id).maybeSingle();
  if(linkError)throw linkError;
  if(!link){const e=new Error('No Mother Agency / Partner Portal is linked to this account.');e.statusCode=403;throw e;}

  const {data:partner,error:partnerError}=await admin.from('partner_agencies')
    .select('*').eq('organization_id',organization.id).eq('id',link.partner_agency_id).maybeSingle();
  if(partnerError)throw partnerError;
  if(!partner||partner.portal_enabled===false){const e=new Error('Mother Agency / Partner Portal access is disabled.');e.statusCode=403;throw e;}

  const {data:pendingMember,error:memberError}=await admin.from('organization_members')
    .select('*').eq('organization_id',organization.id).eq('user_id',user.id)
    .eq('member_type','partner').in('status',['active','invited']).maybeSingle();
  if(memberError)throw memberError;
  let member=pendingMember||null;
  if(member?.status==='invited'){
    const {data:activated,error:activationError}=await admin.from('organization_members')
  ...
  const {data:profile,error:profileError}=await admin.from('profiles').select('*').eq('user_id',user.id).maybeSingle();
  if(profileError)throw profileError;
  return {admin,organization,member,profile,partnerAgencyId:link.partner_agency_id,partner,link};
}
