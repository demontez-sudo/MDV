...
(function(){
'use strict';if(window.__CAVYRE_MODEL_CONTROL_161219__)return;window.__CAVYRE_MODEL_CONTROL_161219__=1;
...
function accessPanel(a,temp){
 ...
 return '<div class="cvyModelCtl-accessgrid"><div><span>Status</span><b>'+esc(a.status||'—')+'</b></div><div><span>Authentication</span><b>'+esc(a.authentication||'—')+'</b></div><div><span>Login Email</span><b>'+esc(a.email||a.suggested_email||'—')+'</b></div><div><span>Last Login</span><b>'+esc(a.last_login?new Date(a.last_login).toLocaleString():'—')+'</b></div></div>'+
 (!linked?'<div class="cvyModelCtl-repair"><b>LOGIN IS NOT LINKED</b><p>Create or reconnect this model’s CAVYRE login without changing the model profile.</p>'+field('Model Login Email','cvy-access-email',a.suggested_email||a.email||'','email')+'<button class="primary" data-link>Repair / Create Login</button></div>':
 ...
 (temp?'<div class="cvyModelCtl-temp"><span>DISPLAY ONCE</span><code>'+esc(temp)+'</code><button data-copy>Copy Login Details</button></div>':'')+'<div class="cvyModelCtl-error" data-error></div>';
}
...
window.CAVYRE_MODEL_CONTROL={release:'16.12.19',editProfile:editProfile,openAccess:openAccess};
document.documentElement.setAttribute('data-cavyre-model-control','16.12.19');
})();