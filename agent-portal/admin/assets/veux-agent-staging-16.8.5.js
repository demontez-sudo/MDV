...
function withTimeout(p,ms,msg){var t;return Promise.race([p,new Promise(function(_,rej){t=setTimeout(function(){rej(new Error(msg))},ms)})]).finally(function(){clearTimeout(t)})}
async function signIn(){loginError('');loginBusy(true);try{var email=((qs('veuxEmail')||{}).value||'').trim().toLowerCase(),password=(qs('veuxPassword')||{}).value||'';if(!email||!password)throw new Error('Enter your email and password.');var client=requireClient();loginBusy(true,'Verifying…');var r=await withTimeout(client.auth.signInWithPassword({email:email,password:password}),20000,'Sign-in is taking too long. Close any other open CAVYRE tabs, refresh this page, and try again.');if(r.error)throw r.error;state.session=r.data.session;await ensureMfa();await withTimeout(openSession(true),60000,'Signed in, but the workspace is taking too long to load. Refresh and try again.');}catch(e){loginError(e.message||'Sign in failed.');loginBusy(false);}}
function askMfaCode(){return new Promise(function(resolve){var o=document.createElement('div');o.id='veux-mfa-modal';o.style.cssText='position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.78);display:flex;align-items:center;justify-content:center;padding:20px';o.innerHTML='<form style="width:min(420px,100%);background:#0d0d14;border:1px solid #7b4bd8;border-radius:12px;padding:28px;color:#f4f0ff;font-family:Inter,Arial,sans-serif"><div style="font-size:10px;letter-spacing:.24em;color:#b58cff;text-transform:uppercase">VEUX Security</div><h2 style="font:400 26px Georgia,serif;margin:8px 0 6px">Enter your code</h2><p style="font-size:13px;color:#a9a3bd;margin:0 0 16px">Open your authenticator app and enter the 6-digit code.</p><input inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="000000" style="width:100%;box-sizing:border-box;padding:14px;font-size:22px;letter-spacing:.4em;text-align:center;background:#05050a;color:#fff;border:1px solid #4b3a78;border-radius:8px"><div style="display:flex;gap:10px;margin-top:16px"><button type="button" data-c style="flex:1;padding:12px;border:1px solid #4b3a78;background:transparent;color:#ddd;border-radius:8px;cursor:pointer">Cancel</button><button type="submit" style="flex:1;padding:12px;border:0;background:#7b4bd8;color:#fff;border-radius:8px;cursor:pointer">Verify</button></div></form>';document.body.appendChild(o);var i=o.querySelector('input');setTimeout(function(){i.focus()},30);function done(v){o.remove();resolve(v)}o.querySelector('form').onsubmit=function(e){e.preventDefault();done(i.value.trim())};o.querySelector('[data-c]').onclick=function(){done(null)}})}
async function ensureMfa(){var client=requireClient();var a=await client.auth.mfa.getAuthenticatorAssuranceLevel();if(a.error)throw a.error;if(a.data&&a.data.nextLevel==='aal2'&&a.data.currentLevel!=='aal2'){var f=await client.auth.mfa.listFactors();if(f.error)throw f.error;var factor=(f.data&&f.data.totp||[]).find(function(x){return x.status==='verified';});if(!factor)throw new Error('MFA is required but no verified authenticator is available.');var code=await askMfaCode();if(!code)throw new Error('MFA verification cancelled.');var ch=await client.auth.mfa.challenge({factorId:factor.id});if(ch.error)throw ch.error;var vr=await client.auth.mfa.verify({factorId:factor.id,challengeId:ch.data.id,code:String(code).trim()});if(vr.error)throw vr.error;state.session=(await client.auth.getSession()).data.session;}}
...
async function signOut(){try{var _b=document.getElementById('vx139-status');if(_b)_b.style.display='none'}catch(_e){}try{if(state.client&&state.client.auth)await state.client.auth.signOut();}catch(_e){}clearApiCache();state.session=null;state.bootstrap=null;state.org=null;window.whoKey=null;rewriteLogin();var app=qs('app');if(app){app.classList.remove('on');app.style.display='none';}var login=qs('login');if(login)login.style.display='';if(document.body)delete document.body.dataset.veuxSessionReady;}
async function init(){if(initStarted)return;initStarted=true;rewriteLogin();loginBusy(true,'Connecting…');try{var cfg=await loadPublicConfig();state.publicConfig=cfg;var forgot=qs('veuxForgotBtn');if(forgot&&cfg.email_delivery_configured===false){forgot.disabled=true;forgot.textContent='Password reset temporarily offline';forgot.title='Email delivery is not configured for this deployment';}if(!window.supabase||!window.supabase.createClient)throw new Error('Supabase client library did not load');state.client=window.supabase.createClient(cfg.supabase_url,cfg.supabase_publishable_key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,storageKey:'veux-agent-mogy-auth-v1'}});loginBusy(false);window.VEUX_SUPABASE_CLIENT=state.client;state.client.auth.onAuthStateChange(function(evt,session){state.session=session;if(evt==='PASSWORD_RECOVERY'||(evt==='SIGNED_IN'&&(passwordSetupMode||urlWantsPasswordSetup())))setTimeout(renderPasswordSetup,0);if(evt==='SIGNED_OUT')rewriteLogin();});var recoveryToken='';try{recoveryToken=new URL(location.href).searchParams.get('recovery_token')||'';}catch(_e){}if(recoveryToken){var vr=await state.client.auth.verifyOtp({token_hash:recoveryToken,type:'recovery'});if(vr.error)throw new Error('This recovery link is invalid or expired. Request one new link from the Agency Portal.');state.session=vr.data&&vr.data.session||null;passwordSetupMode=true;initialAuthUrl=location.href;try{history.replaceState({},document.title,location.pathname);}catch(_e){}}else{var sr=await state.client.auth.getSession();if(sr.error)throw sr.error;state.session=sr.data.session;if(!state.session){try{var code=new URL(location.href).searchParams.get('code');if(code){var ex=await state.client.auth.exchangeCodeForSession(code);if(ex.error)throw ex.error;state.session=ex.data.session;passwordSetupMode=true;}}catch(_e){}}}if(state.session){if(passwordSetupMode||urlWantsPasswordSetup()){renderPasswordSetup();return;}await ensureMfa();await openSession(true);}}catch(e){console.error('[VEUX 16.0] Agency bootstrap error',e);loginError(e.message||'VEUX Agency Portal could not start.');loginBusy(false);}}
