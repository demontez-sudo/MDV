/* CAVYRE 16.12.15 — Model Access + Profile Control */
(function(){
'use strict';if(window.__CAVYRE_MODEL_CONTROL_161215__)return;window.__CAVYRE_MODEL_CONTROL_161215__=1;
...
function modelId(){return window._veuxV10ModelId||window.__CURRENT_MODEL_ID__||null}
...
async function editProfile(){
 ...
 try{
  ...
  var body='<div class="cvy161215-grid">'+
   ...
   '<div class="cvy161215-actions"><button data-cancel>Cancel</button><button class="primary" data-save>Save Profile</button></div><div class="cvy161215-error" data-error></div>';
  var root=shell('cvy-model-profile-161215','MODEL 360 · AGENT CONTROL','Edit Model Profile',body);
  var g=root.querySelector('#cvy-m-gender');if(g)g.value=m.gender||'';
  root.querySelector('[data-cancel]').onclick=function(){close('cvy-model-profile-161215')};
  root.querySelector('[data-save]').onclick=async function(){var b=this,err=root.querySelector('[data-error]');b.disabled=true;err.textContent='';try{
   ...
   close('cvy-model-profile-161215');if(window.toast)toast('✓ Model profile updated and verified');if(window.renderModel360156)renderModel360156();
...
function accessPanel(a,temp){
 ...
 return '<div class="cvy161215-accessgrid"><div><span>Status</span><b>'+esc(a.status||'—')+'</b></div><div><span>Authentication</span><b>'+esc(a.authentication||'—')+'</b></div><div><span>Login Email</span><b>'+esc(a.email||a.suggested_email||'—')+'</b></div><div><span>Last Login</span><b>'+esc(a.last_login?new Date(a.last_login).toLocaleString():'—')+'</b></div></div>'+
 (!linked?'<div class="cvy161215-repair"><b>LOGIN IS NOT LINKED</b><p>Create or reconnect this model’s CAVYRE login without changing the model profile.</p>'+field('Model Login Email','cvy-access-email',a.suggested_email||a.email||'','email')+'<button class="primary" data-link>Repair / Create Login</button></div>':
 '<div class="cvy161215-actions"><button class="primary" data-temp>Generate Temporary Password</button><button data-reset>Send Reset Email</button><button data-toggle>'+(dis?'Enable Portal Access':'Disable Portal Access')+'</button></div>')+
 (temp?'<div class="cvy161215-temp"><span>DISPLAY ONCE</span><code>'+esc(temp)+'</code><button data-copy>Copy Login Details</button></div>':'')+'<div class="cvy161215-error" data-error></div>';
}
async function openAccess(temp){
 var id=modelId();if(!id)return alert('Select a model first.');
 var existing=document.getElementById('cvy-model-access-161215'),root=existing||shell('cvy-model-access-161215','MODEL PORTAL · AUTHENTICATION','Account & Access','<div data-body>Loading…</div>');
 try{
 ...
 }catch(e){(root.querySelector('[data-body]')||root.querySelector('main')).innerHTML='<div class="cvy161215-error">'+esc(e.message||e)+'</div>'}
}
...
function inject(){
 var p=document.getElementById('p-modelpage');if(!p||!p.classList.contains('on')||!modelId())return;
 var target=p.querySelector('.v155-head-actions,.v152-head-actions,.v156-head-actions,.v144-model-top-actions,.v155-model-actions');
 if(!target)return;
 if(!target.querySelector('[data-cvy-edit-model]')){var e=document.createElement('button');e.type='button';e.className='v152-btn';e.dataset.cvyEditModel='1';e.textContent='Edit Profile';e.onclick=editProfile;target.prepend(e)}
 if(!target.querySelector('[data-cvy-model-access]')){var a=document.createElement('button');a.type='button';a.className='v152-btn';a.dataset.cvyModelAccess='1';a.textContent='Account & Access';a.onclick=function(){openAccess()};target.appendChild(a)}
}
document.addEventListener('click',function(e){var b=e.target&&e.target.closest&&e.target.closest('button,a,[role="button"]');if(!b)return;var t=String(b.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();if(t==='edit profile'){e.preventDefault();e.stopImmediatePropagation();editProfile()}if(t==='account & access'||t==='portal access'){e.preventDefault();e.stopImmediatePropagation();openAccess()}},true);
...
window.CAVYRE_MODEL_CONTROL={release:'16.12.15',editProfile:editProfile,openAccess:openAccess};
document.documentElement.setAttribute('data-cavyre-model-control','16.12.15');
})();