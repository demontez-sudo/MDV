...
publishVeraSignals(p);var total=p.models.length,readyPct=total?Math.round(p.ready/total*100):100,ss=series(p.cal),mx=Math.max.apply(null,ss.map(function(x){return x.v}))||1,bars=ss.map(function(x){var dl=new Date(x.k+'T12:00:00').toLocaleDateString([],{weekday:'long',month:'short',day:'numeric'});return'<button type="button" class="vx17-bar" data-vx17-day="'+x.k+'" title="'+dl+' · '+x.v+' scheduled" aria-label="'+dl+', '+x.v+' scheduled. Open day." style="--h:'+Math.max(18,Math.round(x.v/mx*100))+'%"><em>'+x.v+'</em><span>'+x.l+'</span></button>'}).join(''),sig=priority(p).map(function(s){return'<button class="vx17-signal" data-vx17-nav="'+s[3]+'"><small>'+esc(s[0])+'</small><b>'+esc(s[1])+'</b><span>'+esc(s[2])+'</span></button>'}).join(''),value=p.bookings.reduce(function(n,x){return n+amount(x)},0)+p.events.reduce(function(n,x){return n+amount(x)},0),hero=MODE==='desk'?'What needs action right now?':'What needs attention today?',h='<div class="vx17-command" data-mode="'+MODE+'">';
...
h+='<div class="vx17-main-grid"><section class="vx17-panel '+(MODE==='desk'?'vx17-desk-panel':'vx17-overview-panel')+'"><header><h3>'+(MODE==='desk'?'Agency Desk Queue':'Agency Overview')+'</h3><small>Live connected workspace</small></header><div class="vx17-kpis"><div class="vx17-kpi" role="button" tabindex="0" data-vx17-nav="'+(MODE==='desk'?'tasksconsolidated':'calendar')+'"><small>'+(MODE==='desk'?'Open Tasks':'Bookings')+'</small><b>'+(MODE==='desk'?p.tasks.open.length:p.bookings.length)+'</b><span>'+(MODE==='desk'?p.tasks.urgent.length+' high priority':'This 8-day window')+'</span></div><div class="vx17-kpi" role="button" tabindex="0" data-vx17-nav="'+(MODE==='desk'?'calendar':'multipackage')+'"><small>'+(MODE==='desk'?'Conflicts':'Packages')+'</small><b>'+(MODE==='desk'?p.conflicts:arr(p.packs.packages).length)+'</b><span>'+(MODE==='desk'?'Calendar risk':'Client submissions')+'</span></div><div class="vx17-kpi" role="button" tabindex="0" data-vx17-nav="'+(MODE==='desk'?'approvals':'roster')+'"><small>'+(MODE==='desk'?'Approvals':'Active Talent')+'</small><b>'+(MODE==='desk'?p.tasks.approvals.length:total)+'</b><span>'+(MODE==='desk'?'Human decisions':readyPct+'% ready')+'</span></div><div class="vx17-kpi" role="button" tabindex="0" data-vx17-nav="financelegal"><small>'+(MODE==='desk'?'Outstanding':'Calendar Value')+'</small><b>'+(MODE==='desk'?money(p.finance.outstanding,'USD'):money(value,'USD'))+'</b><span>'+(MODE==='desk'?'Finance watch':'Connected pay')+'</span></div></div>'+(MODE==='desk'?'<div class="vx17-desk-queue">'+desk(p)+'</div>':'<div class="vx17-bars">'+bars+'</div>')+'</section>';
h+='<section class="vx17-panel vx17-priority-panel"><header><h3>Priority Stream</h3><small>Why each signal matters</small></header><div class="vx17-priority">'+sig+'</div></section></div><div class="vx17-smart-grid">'+smartCards()+'</div>';h+='<section class="vx161061-mobility"><header><div><small>Vera · Mobility Conflict Intelligence</small><h3>Operational Readiness</h3></div><button data-vx17-nav="visa">Open Travel & Visa</button></header>'+(p.mobility&&p.mobility.items.length?p.mobility.items.slice(0,4).map(function(x){var b=x.booking||{},buf=x.arrival_buffer_minutes;return '<article class="'+esc(x.severity)+'"><div><small>'+esc(String(x.severity).replaceAll('_',' '))+' · '+esc(x.code)+'</small><b>'+esc(b.title||'Booking mobility')+'</b><span>'+esc(x.message)+'</span></div><div class="vx161061-mob-meta"><em>'+(buf==null?'No buffer':buf<0?'Late '+Math.abs(Math.round(buf/60*10)/10)+'h':Math.floor(buf/60)+'h '+(buf%60)+'m buffer')+'</em><button data-vx17-nav="visa">'+esc(x.recommended_action)+'</button></div></article>'}).join(''):'<div class="vx161061-clear"><b>Mobility clear</b><span>No booking-level travel/visa conflict is active.</span></div>')+'</section>';
...
function bind(el){el.querySelectorAll('[data-vx17-nav]').forEach(function(b){b.onclick=function(){focusRecord(b);nav(b.dataset.vx17Nav)}});el.querySelectorAll('[data-vx17-vera]').forEach(function(b){b.onclick=openVera});el.querySelectorAll('[data-vx17-day]').forEach(function(b){b.onclick=function(){try{var st=window.VEUX_CALENDAR_ENGINE&&window.VEUX_CALENDAR_ENGINE.state;if(st){st.anchor=new Date(b.dataset.vx17Day+'T12:00:00');st.view='day';st.selected=null;}}catch(e){}nav('calendar')}});el.querySelectorAll('.vx17-kpi[data-vx17-nav]').forEach(function(k){k.onkeydown=function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();k.click()}}});el.querySelectorAll('[data-vx17-mode]').forEach(function(b){b.onclick=function(){MODE=b.dataset.vx17Mode;renderOverview(document.getElementById('p-overview')||el)}})}
