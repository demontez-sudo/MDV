...
function displayLabel(value){
  ...
  var exact={
    'pr':'PR','ny':'NY','nyc':'NYC','nyfw':'NYFW','pfw':'PFW',
    'pdf':'PDF','url':'URL','api':'API','ai':'AI','crm':'CRM',
    'mfa':'MFA','vip':'VIP','usd':'USD','eur':'EUR','gbp':'GBP',
    'id':'ID','dob':'DOB','ssn':'SSN','vat':'VAT',
    ...
    'model_portal':'Model Portal',
    'mother_agency_portal':'Mother Agency Portal'
  };
  var key=raw.toLowerCase();
  if(exact[key])return exact[key];

  // Preserve already-designed display copy.
  if(/[A-Z]/.test(raw)&&!/_/.test(raw))return raw;

  return raw.replace(/[_-]+/g,' ').replace(/\s+/g,' ').split(' ').map(function(word){
    var low=word.toLowerCase();
    if(exact[low])return exact[low];
    if(/^(and|or|of|to|for|in|on|at|by|with)$/i.test(word))return low;
...
function eligible(s){
  return s && s.tagName==='SELECT' && !s.multiple && !(s.size>1) && !s.dataset.vxSelectIgnore &&
         !s.classList.contains('v16845-native') && !s.closest('.v16845-picker');
}
...
function sync(s){
  ...
  if(val)val.textContent=selected?displayLabel(selected.textContent):'Select';
  if(btn){
...
function rebuild(s){
  ...
  entries.forEach(function(x){
    if(x.kind==='group'){
      html+='<div class="vx-select-group">'+esc(displayLabel(x.label))+'</div>';
      return;
    }
    var o=x.option;
    html+='<button type="button" class="vx-select-option" role="option" data-value="'+esc(o.value)+'" data-search="'+esc(((o.textContent||'')+' '+displayLabel(o.textContent)+' '+(x.group||'')+' '+displayLabel(x.group||'')).toLowerCase())+'" '+(o.disabled?'disabled':'')+'>'+
      '<span>'+esc(displayLabel(o.textContent))+'</span></button>';
  });
