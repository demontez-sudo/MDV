...
export const handler = async event => {
  ...
  try{
    ...
    const [models,profiles,measurements,selectedMedia,publicMedia]=await Promise.all([
      ...
      modelIds.length?rows(admin.from('model_public_profiles').select('model_id,published,headline,bio,show_measurements,show_market,show_socials,show_agent_contact,contact_name,contact_email,contact_phone,metadata').eq('organization_id',link.organization_id).in('model_id',modelIds)):[],
