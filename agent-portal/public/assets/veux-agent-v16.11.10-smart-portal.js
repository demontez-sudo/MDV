...
if(window.__VEUX_SMART_PORTAL_161110__)return;window.__VEUX_SMART_PORTAL_161110__=true;
var MODE='overview';
var WAVES={
...
h+='<section class="vx161061-mobility"><header><div><small>Vera · Mobility Conflict Intelligence</small><h3>Operational Readiness</h3></div><button data-vx17-nav="globalmobility">Open Travel & Visa</button></header>'+(p.mobility&&p.mobility.items.length?p.mobility.items.slice(0,4).map(function(x){var b=x.booking||{},buf=x.arrival_buffer_minutes;return '<article class="'+esc(x.severity)+'"><div><small>'+esc(String(x.severity).replaceAll('_',' '))+' · '+esc(x.code)+'</small><b>'+esc(b.title||'Booking mobility')+'</b><span>'+esc(x.message)+'</span></div><div class="vx161061-mob-meta"><em>'+(buf==null?'No buffer':buf<0?'Late '+Math.abs(Math.round(buf/60*10)/10)+'h':Math.floor(buf/60)+'h '+(buf%60)+'m buffer')+'</em><button data-vx17-nav="globalmobility">'+esc(x.recommended_action)+'</button></div></article>'}).join(''):'<div class="vx161061-clear"><b>Mobility clear</b><span>No booking-level travel/visa conflict is active.</span></div>')+'</section>';
h+='<section class="vx161063-finance"><header><div><small>Vera · Finance Intelligence</small><h3>Revenue Command</h3></div><button data-vx17-nav="financelegal">Open Finance</button></header><div class="vx161063-fin-kpis"><div><small>Outstanding</small><b>'+money(p.finance.outstanding,p.finance.currency)+'</b></div><div><small>Overdue</small><b>'+money(p.finance.overdueTotal,p.finance.currency)+'</b></div><div><small>Due ≤ 7 Days</small><b>'+p.finance.dueSoon.length+'</b></div><div><small>Collection Risk</small><b>'+p.finance.collectionRisk+'%</b></div></div>'+(p.finance.overdue.length?'<div class="vx161063-fin-stream">'+p.finance.overdue.slice(0,4).map(function(x){var amt=Number(x.amount_due||x.balance_due||0),days=x.due_date?Math.max(0,Math.floor((Date.now()-new Date(x.due_date).getTime())/86400000)):0;return '<article><div><small>OVERDUE · '+days+'D</small><b>'+esc(x.invoice_number||'Invoice')+'</b><span>'+money(amt,x.currency||p.finance.currency)+' remains due</span></div><button data-vx17-nav="financelegal">Review Collection →</button></article>'}).join('')+'</div>':'<div class="vx161063-fin-clear"><b>No overdue invoice risk</b><span>'+p.finance.dueSoon.length+' invoice'+(p.finance.dueSoon.length===1?'':'s')+' due within the next seven days.</span></div>')+'</section>';
...
function applyWave(theme,persist){
  ...
  if(s){
    var labels={burntOrange:'Burnt Orange',blueNude:'Blue Nude',greenNude:'Green Nude',purple:'Purple',turquoise:'Turquoise',classic:'Classic',green:'Green',blue:'Blue',red:'Red',white:'White'};
