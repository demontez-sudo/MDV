...
function portalPath(path){if(typeof path!=='string'||path.indexOf('http')===0)return path;/* 16.12.82: API/config/health are origin-root routes backed by Netlify functions. Never prefix them with the branded /admin mount. The mount is only for static Agent Portal files. */if(/^\/(?:api(?:\/|$)|health(?:$|\?)|\.netlify\/functions\/)/i.test(path))return path;if(!portalMount)return path;return path.charAt(0)==='/'?portalMount+path:portalMount+'/'+path;}
...
window.fetch=function(input,init){/* 16.12.82: preserve root API URLs. Legacy branded mounting must not turn /api/* into /admin/api/*. */return nativeFetch(input,init);};
