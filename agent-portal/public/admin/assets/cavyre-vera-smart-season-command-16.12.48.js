/* CAVYRE 16.12.98 — Season Runtime Repair */
(function(){'use strict';
...
function mainContent(){
 ...
 if(S.tab==='movement'){
   ...
   return'<section class="ss48-focus"><header><div><small>SEASON COMMAND</small><h2>Movement</h2><p>'+tr.length+' season-linked travel record'+(tr.length===1?'':'s')+'.</p></div><button class="ss48-primary" data-go="globalmobility">Open Full Desk →</button></header>'+(tr.length?tr.map(function(x){return'<article class="ss48-dev"'+(x.model_id?' data-open-model="'+E(x.model_id)+'"':'')+'><div><h3>'+E(x.models&&x.models.display_name||'Travel')+'</h3><p>'+E(short(x.starts_at))+(x.ends_at?' – '+E(short(x.ends_at)):'')+'</p></div><b>'+E(label(x.status||x.travel_status||'planned'))+'</b><span>'+E(x.destination||x.location||x.city||'Movement record')+'</span></article>'}).join(''):'<div class="ss48-empty big">No travel is linked to this season yet.</div>')+'</section>';
 }
 if(S.tab==='visa'){
   var vm=models().filter(function(m){return readiness(m).some(function(x){return /visa|passport|permit|compliance|work authorization/i.test(String(x.item_type||x.title||x.name||''))})});
   return'<section class="ss48-focus"><header><div><small>SEASON COMMAND</small><h2>Visa & Compliance</h2><p>Compliance readiness tied to season talent.</p></div><button class="ss48-primary" data-go="visa">Open Full Desk →</button></header>'+(vm.length?vm.map(function(m){var items=readiness(m).filter(function(x){return /visa|passport|permit|compliance|work authorization/i.test(String(x.item_type||x.title||x.name||''))});return'<article class="ss48-dev" data-open-model="'+E(m.model_id)+'"><div><h3>'+E(modelName(m))+'</h3><p>'+items.length+' compliance item'+(items.length===1?'':'s')+'</p></div><b>'+Number(m.readiness_percent||0)+'%</b><span>'+items.slice(0,2).map(function(x){return E(label(x.item_type||x.title||x.name||'Compliance'))+': '+E(label(x.status||'missing'))}).join(' · ')+'</span></article>'}).join(''):'<div class="ss48-empty big">No visa/compliance requirements are connected to season talent yet.</div>')+'</section>';
 }
...
window.CAVYRE_SMART_SEASON_161248={render:smartRender,install:install,takeover:takeover,state:S,release:'16.12.98'};
})();