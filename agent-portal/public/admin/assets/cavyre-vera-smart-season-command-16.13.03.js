...
function status(m){var st=String(m.status||'planned').toLowerCase(),r=Number(m.readiness_percent||0),av=availabilityState(m);if(av==='unavailable'||st==='at_risk'||r<60)return'at_risk';if(r>=85||st==='confirmed')return'ready';return'development'}
...
function rows(){
 ...
 return ms.map(function(m){var st=status(m),as=assigned(m),av=availabilityState(m),bk=modelBookings(m.model_id);return'<div class="ss48-row" data-model="'+E(m.model_id)+'"><span class="ss48-check"></span><button class="ss48-person" data-open-model="'+E(m.model_id)+'"><i>'+E(modelName(m).split(/\s+/).map(function(x){return x[0]}).slice(0,2).join(''))+'</i><b>'+E(modelName(m))+'</b></button><span class="ss48-status '+st+'"><i></i>'+E(label(st))+'</span><span class="ss48-ready"><b>'+Number(m.readiness_percent||0)+'%</b><i><em style="width:'+Math.min(100,Number(m.readiness_percent||0))+'%"></em></i></span><span class="ss48-reqs"><span class="ss48-req '+(av==='unavailable'?'bad':'')+'">'+E(label(av))+'</span>'+(bk.length?'<span class="ss48-req">'+bk.length+' booking'+(bk.length===1?'':'s')+'</span>':keyReq(m))+'</span><span class="ss48-shows">'+(as.length?as.slice(0,3).map(function(x){return'<b>'+E(x)+'</b>'}).join(''):'—')+'</span><span>'+E(short(m.arrival_at))+' – '+E(short(m.departure_at))+'</span><button class="ss48-more" data-open-model="'+E(m.model_id)+'">•••</button></div>'}).join('')
}
function veraPanel(){
...
function seasonCalendar(c){if(!c||!c.starts_on||!c.ends_on)return'<div class="ss48-empty big">Season dates are required to build the show calendar.</div>';var a=new Date(c.starts_on),z=new Date(c.ends_on);a.setHours(12,0,0,0);z.setHours(12,0,0,0);var out='<div class="ss48-cal">',guard=0;for(var d=new Date(a);d<=z&&guard<90;d.setDate(d.getDate()+1),guard++){var key=d.toISOString().slice(0,10),ds=shows().filter(function(x){return x.starts_at&&new Date(x.starts_at).toISOString().slice(0,10)===key});out+='<div class="ss48-day"><small>'+E(d.toLocaleDateString([],{weekday:'short'}))+'</small><b>'+E(d.toLocaleDateString([],{month:'short',day:'numeric'}))+'</b>'+(ds.length?ds.map(function(x){return'<button><strong>'+E(x.title||'Show')+'</strong><span>'+E(x.companies&&x.companies.name||x.location||label(x.status))+'</span></button>'}).join(''):'<em>OPEN DAY</em>')+'</div>'}return out+'</div>'}
...
function mainContent(){
 ...
 if(S.tab==='movement'){
   var tr=((S.data&&S.data.travel)||[]).filter(function(x){return !c.starts_on||!x.starts_at||new Date(x.starts_at)>=new Date(c.starts_on)&&(!c.ends_on||new Date(x.starts_at)<=new Date(c.ends_on))});
...
function bind(el){
 el.querySelectorAll('[data-ss-season]').forEach(function(b){b.onclick=function(e){if(e){e.preventDefault();e.stopPropagation()}S.seasonId=b.dataset.ssSeason;S.filter='all';S.query='';render(el)}});
...
window.CAVYRE_SMART_SEASON_161248={render:smartRender,install:install,takeover:takeover,state:S,release:'16.13.03'};
})();